...
def findBestMoveMiniMaxABTTID(gs, possibleMoves):
    ...
    transpositionTable = defaultdict(list)
    startTime = time.time()
    for currentMaxDepth in range(1, 7):
        ...
        print("currentMaxDepth",currentMaxDepth)
        findBestMoveMiniMaxABTTIDHelper(
            gs, possibleMoves, currentMaxDepth, gs.redToMove, nextMove, alpha, beta, transpositionTable)
    print("visited node number: ", nextMove[1])
    print("execution time: ", time.time() - startTime)
    return nextMove[0]

def findBestMoveMiniMaxABTTIDHelper(gs, possibleMoves, d, redToMove, nextMove, alpha, beta, transpositionTable):
    nextMove[1] += 1
    alphaOrig = alpha
    # print("doing")
    ...
    if redToMove:
        ...
        for move in possibleMoves:
            ...
            nextMoves = gs.getAllPossbileMoves()
            nextMoves.sort()
            score = findBestMoveMiniMaxABTTIDHelper(
                gs, nextMoves, d - 1, False, nextMove, alpha, beta, transpositionTable)
            if score > maxScore:
                maxScore = score
                if d == maxDepth:
